...
def get_data(filters):
	...
	details = frappe.db.sql('''select op.customer_name, op.opportunity_type,op.lead_owner,op.sales_person, op.company_name,op.transaction_date,op.closing_date,op.status,op.sales_stage,op.source
	from `tabOpportunity` as op where %s order by status''' % conditions , filters, as_dict=1)
