...
def get_last_selling_prices_of_customer(item_code='', customer=''):
    last_purchase_price = frappe.db.sql(
        """select rate,posting_date,si.supplier from `tabPurchase Invoice Item` sid inner join `tabPurchase Invoice` si on sid.parent= si.name where sid.item_code = '{}' and si.docstatus != 2 order by si.posting_date DESC limit 5""".format(
            item_code))

    last_customer_prices = frappe.db.sql(
        """select rate,posting_date,si.customer from `tabSales Invoice Item` sid inner join `tabSales Invoice` si on sid.parent= si.name where si.customer != '{}' and sid.item_code = '{}' and si.docstatus != 2 order by si.posting_date DESC limit 3""".format(
            customer, item_code))
    ...
    current_stock = frappe.db.sql(
        """select warehouse,actual_qty from `tabBin` where item_code = '{}'""".format(item_code))
